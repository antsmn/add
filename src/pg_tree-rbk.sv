module pg_tree #(
  ...
  if (W == 1) begin
    ...
    pg #(2) i_pg (.p_i(p_i), .g_i(g_i), .p_o(p_o[1]), .g_o(g_o[1]));
    ...
    for (genvar i = 0; i < H; i += 1) begin
      ...
      pg #(2) i_pg (.p_i(op1), .g_i(og1), .p_o(op2[i]), .g_o(og2[i]));

    end

    // prefix sum W / 2

    pg_tree #(.W(H)) i_pg_tree (.p_i(op2), .g_i(og2), .p_o(op3), .g_o(og3));
    ...
    for (genvar i = 1; i < H; i += 1) begin
      wire [1:0] ep1 = {ep[i], op3[i-1]};
      wire [1:0] eg1 = {eg[i], og3[i-1]};

      pg #(2) i_pg (.p_i(ep1), .g_i(eg1), .p_o(ep2[i]), .g_o(eg2[i]));
