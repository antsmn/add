module carry_increment_rca #(
  ...
  for (genvar i = K - 1; i < W; i += K) begin
    g_rca #(K) i_g_rca (.p_i(p_1[i-:K]), .g_i(g_1[i-:K]), .g_o(g_2[i-:K]));
  end

  assign g_3[K-1:0] = g_2[K-1:0];

  for (genvar i = 2 * K - 1; i < W; i += K) begin
    p_rca #(K) i_p_rca (.p_i(p_1[i-:K]), .p_o(p_2[i-:K]));
    g_inc #(K) i_g_inc (.p_i(p_2[i-:K]), .g_i(g_2[i-:K]), .c_i(g_3[i-K]), .g_o(g_3[i-:K]));
  end
